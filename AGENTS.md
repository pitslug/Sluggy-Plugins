...
- Known blockers: the claude.ai sync does NOT poll GitHub. After a push, Chris opens claude.ai → Manage marketplaces → the three-dot menu on Sluggy-Plugins → Check for updates; removing and re-adding the plugin does not refresh it. Then close and reopen every Claude desktop app instance (each machine) and check for updates on the plugin itself; running sessions never see an edit until that restart; the synced copy on this machine lives under `AppData/Roaming/Claude/local-agent-mode-sessions/.../rpm/plugin_*/` and does not show in `claude plugin list`
...
- Every change to `plugins/code-agents/` bumps `version` in its `plugin.json` AND the matching `version` in `.claude-plugin/marketplace.json` before push, then Chris runs Check for updates in claude.ai (see Validation notes); an unbumped change never reaches an installed copy. Manifest-only wording changes may wait for the next bump.
